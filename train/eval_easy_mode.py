"""
Evaluate models on EASY MODE configuration for 70%+ efficiency target.

This script:
1. Loads the easy mode configuration (5 agents, 20 patches, smaller world)
2. Can evaluate existing Phase 1 model OR calculate new baseline
...
# Easy Mode Configuration (from env_easy_mode.yaml)
EASY_CONFIG = {
    "n_agents": 5,
    "n_patches": 20,
    "width": 20.0,
...
# Theoretical maximum: 5 agents × 2000 steps × 0.08 c_max = 800 total
THEORETICAL_MAX = EASY_CONFIG["n_agents"] * EASY_CONFIG["episode_len"] * EASY_CONFIG["c_max"]
...
def evaluate_model(model_path: str, n_episodes: int = 100):
    ...
    if stats['mean_efficiency'] >= 70.0:
        print("🎉 🎉 🎉 SUCCESS! Achieved 70%+ efficiency target! 🎉 🎉 🎉")
    elif stats['mean_efficiency'] >= 60.0:
        print("✅ Great performance! Almost at 70% target.")
    elif stats['mean_efficiency'] >= 50.0:
        print("⭐ Good performance, but below 70% target. Consider retraining.")
    else:
        print("⚠️  Below 50% efficiency. Retraining on easy config recommended.")
    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Evaluate model on easy mode configuration")
    parser.add_argument(
        "--model",
        type=str,
        default="models/ppo_final/phase1/model",
        help="Path to model (default: Phase 1 model trained on 5 agents)"
    )
    parser.add_argument(
        "--episodes",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/easy_mode_evaluation.json",
    ...
    if not Path(args.model + ".zip").exists():
        ...
        print("  - models/ppo_final/phase1/model (5 agents)")
        print("  - models/ppo_final/final/model (10 agents)")
        return
