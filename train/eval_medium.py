...
# Load Medium Mode Configuration from YAML
CONFIG_PATH = Path(__file__).parent.parent / "configs" / "env_medium.yaml"
...
def evaluate_model(model_path: str, n_episodes: int = 100, vecnormalize_path: str = None):
    ...
    print(f"Configuration: Medium Mode (10 agents, 18 patches, 23×23 world)")
...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="models/ppo_medium/final_model",
        help="Path to trained PPO model (default: models/ppo_medium/final_model)"
    )
    parser.add_argument(
        "--episodes",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/medium_evaluation.json",
    ...
    if not Path(args.model + ".zip").exists():
        ...
        print("  python -m train.train_ppo --config configs/env_medium.yaml --output models/ppo_medium")
        return
