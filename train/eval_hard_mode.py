...
This script:
1. Loads the hard mode configuration (15 agents, 8 patches, larger world)
...
# Hard Mode Configuration (from env_hard_mode.yaml)
HARD_CONFIG = {
    "n_agents": 15,
    "n_patches": 8,
    "width": 60.0,
...
# Theoretical maximum: 15 agents × 3000 steps × 0.05 c_max = 2250 total
...
def evaluate_model(model_path: str, n_episodes: int = 100, vecnormalize_path: str = None):
    ...
    print(f"Configuration: Hard Mode (15 agents, 8 patches, 60×60 world)")
    print(f"Episodes: {n_episodes}")
    print(f"Theoretical max: {THEORETICAL_MAX:.0f} intake")
    print(f"Realistic target: 40-60% efficiency (900-1350 intake)")
    print("=" * 80)
    print()
    ...
    for ep in range(n_episodes):
        ...
        # Additional metrics for hard mode
    ...
    tiers = {
        "excellent_60plus": len([e for e in all_episodes if e["efficiency_percent"] >= 60]),
        "great_50_60": len([e for e in all_episodes if 50 <= e["efficiency_percent"] < 60]),
        "good_40_50": len([e for e in all_episodes if 40 <= e["efficiency_percent"] < 50]),
        "ok_30_40": len([e for e in all_episodes if 30 <= e["efficiency_percent"] < 40]),
        "below_30": len([e for e in all_episodes if e["efficiency_percent"] < 30]),
    }
    ...
    print("=" * 80)
    print(f"🏆 Excellent (≥60%):  {tiers['excellent_60plus']:3d} episodes ({tiers['excellent_60plus']/n_episodes*100:5.1f}%)")
    print(f"🌟 Great (50-60%):    {tiers['great_50_60']:3d} episodes ({tiers['great_50_60']/n_episodes*100:5.1f}%)")
    print(f"⭐ Good (40-50%):     {tiers['good_40_50']:3d} episodes ({tiers['good_40_50']/n_episodes*100:5.1f}%)")
    print(f"   OK (30-40%):       {tiers['ok_30_40']:3d} episodes ({tiers['ok_30_40']/n_episodes*100:5.1f}%)")
    print(f"   Below 30%:         {tiers['below_30']:3d} episodes ({tiers['below_30']/n_episodes*100:5.1f}%)")
    print("=" * 80)
    print()

    # Performance assessment for hard mode
    if stats['mean_efficiency'] >= 60.0:
        print("🎉 🎉 🎉 EXCELLENT! Achieved 60%+ efficiency on HARD MODE! 🎉 🎉 🎉")
    elif stats['mean_efficiency'] >= 50.0:
        print("✅ Great performance! 50%+ efficiency on hard mode is impressive.")
    elif stats['mean_efficiency'] >= 40.0:
        print("⭐ Good performance! 40%+ efficiency shows learning, but room for improvement.")
    elif stats['mean_efficiency'] >= 30.0:
        print("📈 Moderate performance. The model is learning but needs more training.")
    else:
        print("⚠️  Below 30% efficiency. Consider longer training or hyperparameter tuning.")
    print()

    # Compile full results
    results = {
        "model_path": model_path,
        "configuration": "hard_mode",
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to trained PPO model (e.g., models/ppo_hard_mode/final_model)"
    )
    parser.add_argument(
        "--episodes",
        type=int,
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="results/hard_mode_evaluation.json",
    ...
    if not Path(args.model + ".zip").exists():
        ...
        print("  python -m train.train_ppo --config configs/env_hard_mode.yaml --output models/ppo_hard_mode")
        return
